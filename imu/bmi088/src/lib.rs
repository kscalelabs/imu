...
impl Bmi088 {
    ...
    pub fn new(i2c_path: &str) -> Result<Self, Error> {
        ...
        println!("Accel chip ID verified: 0x{:02X}", chip_id);
        
        // Soft reset, power-up, and configure accelerometer.
        accel_i2c.smbus_write_byte_data(AccelRegisters::SoftReset as u8, Constants::SoftResetCmd as u8)?;
    ...
    pub fn read_raw_accelerometer(&mut self) -> Result<Vector3, Error> {
        let mut buf = [0u8; 6];
        for i in 0..6 {
            buf[i] = self.accel_i2c.smbus_read_byte_data(AccelRegisters::AccelXLsb as u8 + i as u8)?;
        }
    ...
    pub fn read_raw_gyroscope(&mut self) -> Result<Vector3, Error> {
        let mut buf = [0u8; 6];
        for i in 0..6 {
            buf[i] = self.gyro_i2c.smbus_read_byte_data(GyroRegisters::XLsb as u8 + i as u8)?;
        }
...
impl Bmi088Reader {
    ...
    pub fn new(i2c_bus: &str) -> Result<Self, Error> {
        println!("Initializing LOCAL BMI088 implementation...");
    ...
    fn start_reading_thread(&self, i2c_bus: &str, command_rx: mpsc::Receiver<ImuCommand>) -> Result<(), Error> {
                ...
                if let Ok(cmd) = command_rx.try_recv() {
                    match cmd {
                        ImuCommand::SetAccelRange(range) => {
                            ...
                            let _ = imu.accel_i2c.smbus_write_byte_data(AccelRegisters::AccRange as u8, range as u8);
                        }
                        ImuCommand::SetGyroRange(range) => {
                            imu.gyro_range = range;
                            let _ = imu.gyro_i2c.smbus_write_byte_data(GyroRegisters::Range as u8, range as u8);
                        }
                        ImuCommand::Reset => {
                        ...
                        ImuCommand::Stop => {
                            if let Ok(mut w) = running.write() { *w = false; }
                            break;
                        }
                    }
                }
