...
fn main() -> Result<(), String> {
    ...
    let imu_reader = ImuReader::new("can0", 1, 1).map_err(|e| format!("Failed to initialize IMU reader: {}", e))?;

    // Continuously read and print IMU data
    loop {
        let data = imu_reader.get_data().map_err(|e| format!("Failed to get IMU data: {}", e))?;
        println!(
